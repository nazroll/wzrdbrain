...
`wzrdbrain` is a Python library designed to generate random trick combinations for wizard skating. It can create sequences of tricks, ensuring that the transitions between them are logical. Users can also create individual tricks with specific attributes. The project also includes a JavaScript version of the library, which is automatically generated from the Python source code. The current version of the project is 0.2.2.
...
The project has been recently updated to version 0.2.2. This update brings the Python and JavaScript data definitions back in sync by adding new moves like stunami, ufo swivel, toe pivot, and heel pivot to the JavaScript reference implementation. It also fixes a CI bug related to the black formatting dependency on older Python versions, refines the Ruff configuration, and adds more test coverage for edge cases and specific move rules.
